  ...
  if (window.bounceDeck?.companionChat) {
    try {
      ...
      if (result.configured && result.text) {
        return { text: result.text, mode: 'llm', model: result.model };
      }
    } catch (error) {
...
function localReply(context: CompanionContext) {
  const { snapshot, track, currentTime, personality } = context;
  const strongest = [
  ...
  if (personality === 'producer') {
    return `At ${stamp} in “${track}”, the analyzer has the ${strongest} carrying the most energy and the overall level is ${energy}. I’m on local-listener mode right now, so I won’t invent arrangement details I can’t actually measure.`;
  }

  if (personality === 'listener') {
    return `I’m with you at ${stamp}. The analyzer says the ${strongest} is leading and the track is ${energy}. I’ll keep quiet unless you pull me in.`;
  }

  return `I’m right here at ${stamp} in “${track}”. The ${strongest} is winning the analyzer right now and the whole thing is ${energy}. Hook up the LLM bridge and I get a much bigger musical brain; until then I’m the tiny honest goblin watching the meters.`;
}
